...
# ✨ Mapa para converter o ID da ocasião em texto
OCCASION_MAP = {
    1: "Dia a Dia / Leve",
    2: "Social / Encontro",
...
async def get_wine_recommendations(questionnaire: WineQuestionnaire):
    """
    Rota pública: retorna uma lista de vinhos recomendados
    ...
    try:
        user_profile = wine_recommender_instance.process_user_answers(
            questionnaire.model_dump()
        )

        recs = wine_recommender_instance.get_recommendations_from_profile(
            user_profile, num_recommendations=5
        )
    ...
    for r in recs:
        wine_data = {
            ...
            # campos extras
            "harmonizacao": r.get("harmonizacao"),
            ...
            "user_occasion": OCCASION_MAP.get(
                questionnaire.ocasiao,
                "Não especificada"
            ),
